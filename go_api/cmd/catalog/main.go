...
func main() {
	db, err := sql.Open("mysql", "root:root@tcp(localhost:3306)/complete_ecommerce")
	if err != nil {
		panic(err.Error())
	}
	...
	webProductHandler := webserver.NewWebProductHanlder(productService)

	// o c representa as rotas
	c := chi.NewRouter()
